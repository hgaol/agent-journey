...
Replay video is a silent, derived H.264 MP4 generated locally from projected Replay frames. The export records its selected local rendering engine, quality, speed, frame rate, streaming mode, simulated-prompt setting and typing speed, Style Pack, and redaction in the rendering itself; simulated cadence is visibly labeled. Temporary raster frames are deleted after encoding. MP4 is not Source Evidence, does not claim Terminal Fidelity, and is not re-importable.
