...
Replay progressively reveals terminal behavior through a Playhead. Entering Replay resets to the first frame, shows that frame before advancing, and starts automatically when the selected mode has autoplay-safe timing; manual-only histories remain paused. Prompt, reasoning, Delivery Trace chunks, tool start/result, state transitions, and Agent Threads appear according to evidenced timestamps. Parallel thread lanes remain visible and long idle gaps are compressed explicitly rather than inventing timing. Untimed Activities within an otherwise timestamped source use visibly labeled Source-Order Placement and retain no observed timestamp. Users may choose event steps, Recorded Streaming when Delivery Traces exist, or explicitly labeled Simulated Streaming; simulation affects presentation only and never changes the Fidelity Manifest. Recorded and simulated modes expose a 0.5×–16× Streaming Speed that is independent from Timeline Speed. An explicit Simulated Prompt Typing option reveals final Human Input in the native composer and visually submits it before subsequent Activities; it is labeled simulation and never sends input to an agent.
