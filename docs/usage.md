...
The **Prompt playback** selector defaults to **simulated typing** and can be changed to instant submission: Human Input appears as a draft in the source-native composer, then moves into the transcript before the response. This is labeled simulation, exposes a separate Slow/Normal/Fast/Very fast Typing Speed, represents no recorded keystrokes, and sends nothing to the source agent.

Recorded and simulated response modes expose a **Streaming speed** selector from 0.5× through 16×. Simulated prompts instead expose an independent **Typing speed** from 0.5× through 4×. Streaming Speed changes only response chunks; the **Timeline speed** selector independently controls timestamp gaps between Activities.
