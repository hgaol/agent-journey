...
Turns are evidenced when native turn IDs exist and otherwise marked inferred. Delivery Trace chunks remain attached to one semantic Activity. Replay frames use evidenced timestamps/chunk offsets. Isolated untimed Activities within a timestamped stream use labeled Source-Order Placement; wholly untimed Interpretations permit manual stepping in evidence-based modes. The user may explicitly opt into Simulated Streaming, which creates deterministic character-reveal frames labeled `simulated` without changing Canonical Activity or the Fidelity Manifest. Review-to-Replay transition resets to frame zero, holds the first frame for 400 ms, auto-starts only when timing is safe, and recreates the Stage iframe so stale Review DOM cannot survive the transition. Streaming Speed scales response chunk transitions, Typing Speed independently scales Simulated Prompt Typing, and Timeline Speed separately scales inter-Activity timestamp gaps. Prompt draft frames omit the Human Input Activity from the projected Stage until a distinct submitted frame, then restore the immutable final input without changing Canonical Activity. Inputs over 1,000 characters skip typing and use one labeled Simulated Prompt Paste frame to insert the complete content, avoiding an unbounded frame plan without pretending the Typing Speed changed. Draft updates use a small iframe message that mutates only composer text; simulated and recorded response chunks replace only their current Activity. Deadline-based playback catches up after slow renders, remaining time is read from a precomputed suffix schedule, and the timeline uses a bounded Activity-level overview rather than materializing every chunk as a DOM control. Session size therefore does not alter selected Typing or Streaming Speed. Long idle intervals are visibly compressed. Remaining Replay Time sums the same scheduled delays used by playback, updates every 100 ms while running, reacts to Timeline, Streaming, and Typing Speed changes, and remains unavailable when timing supports manual stepping only.
