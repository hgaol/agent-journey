...
const productNames={"claude-code":"Claude Code","codex-cli":"Codex","pi":"Pi","github-copilot-cli":"GitHub Copilot CLI"};
...
function renderActivity(activity,annotation,query,selectedActivityId,duration){
  ...
  if(activity.text){const text=node("div","activity-text");appendMarkdown(text,activity.text,query);if(activity.kind==="reasoning"||activity.kind==="context-injection"){const details=node("details","collapsed-activity");details.append(node("summary",null,activity.kind==="reasoning"?"Show reasoning":"Show injected context"),text);if((query&&activity.text.toLocaleLowerCase().includes(query.toLocaleLowerCase()))||getComputedStyle(document.documentElement).getPropertyValue("--stage-expand-reasoning").trim()==="1")details.open=true;container.append(details)}else container.append(text)}
  if(activity.kind==="tool-invocation"){const argument=conciseToolArgument(activity.payload);if(argument){const detail=node("div","tool-argument");appendInline(detail,argument,query);const timeout=toolTimeout(activity.payload);if(timeout)detail.append(node("span","tool-timeout",timeout));container.append(detail)}}
...
function render(documentValue,rendererTree){
  ...
  const activitiesById=new Map(activities.map(activity=>[activity.id,activity]));let lastThread;
  for(const activity of activities){const turn=turns.get(activity.id);if(turn)stage.append(node("div","turn-marker",(turn.boundaryProvenance==="evidenced"?"Evidenced":"Inferred")+" turn"));if(activity.threadId!==lastThread){stage.append(node("div","thread",activity.threadId==="main"?"Main journey":activity.threadId));lastThread=activity.threadId}const invocationId=activity.kind==="tool-result"?activity.links?.find(link=>link.relation==="result-of")?.targetActivityId:undefined;const invocation=invocationId?activitiesById.get(invocationId):undefined;const duration=evidencedDuration(invocation?.timestamp,activity.timestamp);stage.append(renderActivity(activity,annotations.get(activity.evidenceAnchor),documentValue.presentation.searchQuery||"",documentValue.presentation.selectedActivityId,duration))}
  const nativeComposer=node("div","stage-native-composer");nativeComposer.append(node("span",null,"❯"),node("i"));stage.append(nativeComposer);const nativeFooter=node("footer","stage-native-footer");nativeFooter.append(node("span","stage-native-footer-help","← open sidebar · / commands · ? help · tab next tab"),node("span","stage-native-footer-workspace",documentValue.workspace||"workspace unknown"),node("span","stage-native-footer-model",(documentValue.models?.[0]||"model unknown")+(documentValue.gitBranch?" · "+documentValue.gitBranch:"")));stage.append(nativeFooter);
