...
function appendInline(parent,text,query){const pattern=/(\\x60[^\\x60]+\\x60|\\*\\*[^*]+\\*\\*|\\[[^\\]]+\\]\\([^)]+\\)|(?:\\/[A-Za-z0-9_.@~+:-]+)+(?:[A-Za-z0-9_.@~+:-]+)?|(?:[A-Za-z0-9_.@+-]+\\/)+[A-Za-z0-9_.@+:-]*)/g;let cursor=0;for(const match of String(text).matchAll(pattern)){const index=match.index??0;if(index>cursor)appendHighlighted(parent,String(text).slice(cursor,index),query);const raw=match[0];let shown=raw,className="path-token",tag="span";if(raw.charCodeAt(0)===96){shown=raw.slice(1,-1);className="inline-code";tag="code"}else if(raw.startsWith("**")){shown=raw.slice(2,-2);className="markdown-strong";tag="strong"}else if(raw.startsWith("[")){shown=raw.slice(1,raw.indexOf("]"));className="path-token"}const token=node(tag,className);appendHighlighted(token,shown,query);parent.append(token);cursor=index+raw.length}if(cursor<String(text).length)appendHighlighted(parent,String(text).slice(cursor),query)}
function appendMarkdown(parent,text,query){const lines=String(text).split(/\\r?\\n/);let codeLines=null;for(const line of lines){if(line.startsWith(String.fromCharCode(96,96,96))){if(codeLines===null)codeLines=[];else{const pre=node("pre","markdown-code");const code=node("code");appendHighlighted(code,codeLines.join("\\n"),query);pre.append(code);parent.append(pre);codeLines=null}continue}if(codeLines!==null){codeLines.push(line);continue}const heading=/^(#{1,4})\\s+(.*)$/.exec(line);const bullet=/^[-*]\\s+(.*)$/.exec(line);const element=heading?node("h"+Math.min(6,heading[1].length+2),"markdown-heading"):bullet?node("div","markdown-bullet"):node("div","markdown-line");appendInline(element,(bullet?"• ":"")+(heading?.[2]??bullet?.[1]??line),query);parent.append(element)}if(codeLines!==null){const pre=node("pre","markdown-code");pre.textContent=codeLines.join("\\n");parent.append(pre)}}
...
function renderActivity(activity,annotation,query,selectedActivityId){
  const container=node("article","activity");container.dataset.kind=activity.kind;container.dataset.thread=activity.threadId;container.dataset.activityId=activity.id;if(activity.status)container.dataset.status=activity.status;if(activity.id===selectedActivityId)container.classList.add("selected");
  ...
  if(activity.kind==="tool-invocation"){const argument=conciseToolArgument(activity.payload);if(argument){const detail=node("div","tool-argument");appendInline(detail,argument,query);container.append(detail)}}
