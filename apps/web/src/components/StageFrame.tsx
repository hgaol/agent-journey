...
function render(documentValue,rendererTree){
  ...
  const compactWorkspace=String(documentValue.workspace||"workspace unknown").replace(/^\\/home\\/[^/]+(?=\\/|$)/u,"~");stage.append(node("div","stage-native-workspace",compactWorkspace));const nativeComposer=node("div","stage-native-composer");nativeComposer.append(node("span",null,"❯"),node("i"));stage.append(nativeComposer);const nativeFooter=node("footer","stage-native-footer");const nativeHelp=node("span","stage-native-footer-help");nativeHelp.append(node("strong","stage-native-footer-key","←"),document.createTextNode(" open sidebar · "),node("strong","stage-native-footer-key","/"),document.createTextNode(" commands · "),node("strong","stage-native-footer-key","?"),document.createTextNode(" help · "),node("strong","stage-native-footer-key","tab"),document.createTextNode(" next tab"));const nativeModel=node("span","stage-native-footer-model");nativeModel.append(node("span","stage-native-footer-model-name",documentValue.models?.[0]||"model unknown"));const effortValue=[...activities].reverse().map(activity=>activity.payload&&typeof activity.payload==="object"&&!Array.isArray(activity.payload)?activity.payload.reasoningEffort:undefined).find(value=>typeof value==="string");if(effortValue){const effortLabels={xhigh:"Extra High",high:"High",medium:"Medium",low:"Low",minimal:"Minimal",none:"None",max:"Max"};nativeModel.append(node("span","stage-native-footer-effort"," · "+(effortLabels[effortValue]||effortValue)))}if(documentValue.gitBranch)nativeModel.append(node("span","stage-native-footer-branch"," · "+documentValue.gitBranch));nativeFooter.append(nativeHelp,node("span","stage-native-footer-workspace",documentValue.workspace||"workspace unknown"),nativeModel);stage.append(nativeFooter);
